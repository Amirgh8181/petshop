      ...
      {data.map(item =>
        <div key={item._id}
          ...
          <div className='flex flex-col items-center md:items-start md:text-left text-center space-y-2 text-black/70 text-sm md:w-1/3 w-[90%]'>
            <div className='text-black md:text-lg text-base font-bold'>{item.name}</div>
          ...
          <div className='flex flex-col items-center md:text-lg text-base font-bold md:w-1/3 w-[90%] space-y-8'>
            ...
            <div>
              <Link href={`${href}${item._id}`} className='p-4 rounded-full bg-petBlue/70 hover:bg-petBlue text-white transition-all duration-400'>
                Book an Appointment
              </Link>
            </div>
          </div>
